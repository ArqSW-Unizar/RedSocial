    ...
    <div class="main main-raised">
      <div class="section section-basic">
        <div class="container">
          ...
          <md-table>
                  <md-table-row>
                    <md-table-cell> <b>Autor</b></md-table-cell>
                    <md-table-cell> <b>Contenido</b></md-table-cell>
                    <md-table-cell style="width:200px;"> <b>Fecha</b></md-table-cell>
                  </md-table-row>
                  <md-table-row slot="md-table-row" v-for="comentario of listaComentarios" :key="comentario.id_comentario">
 
                      <md-table-cell md-label="usuario1" >
                        ...
                        <md-table-cell md-label="contenido1">
                      ...
                      <md-table-cell md-label="fecha4" style="width:200px;">
...
export default {
  ...
  beforeMount(){
    ...
    this.info()
    var idtema = this.$route.query.id
    let url = 'http://localhost:3000/api/usuario/listaComentarios/' + idtema+ ''
    this.$http.get(url)
      .then(response => {
        if (response.status === 200) {
          ...
          for (var i = 0; i < datos.length; i++) {
            this.listaComentarios.push({ id_comentario: datos[i].id_comentario, usuario: datos[i].usuario, 
            contenido: datos[i].contenido, fecha: datos[i].fecha})
          }
        } 
        else if (response.status === 201){
            this.errores.title = 'Atención'
            this.errores.msg = 'Todavía no existe ningún comentario, se el primero.'
  ...
  methods: {
    info(){
        ...
        let url = 'http://localhost:3000/api/usuario/info/' + this.$route.query.id + ''
        this.$http.post(url)
            .then(response => {
            if (response.status === 200) {
